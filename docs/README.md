...
Download and install [Nodes.js](https://nodejs.org/).
...
1. Set correct `base` in `src/.vuepress/config.js`.
...
2. Set `dest` in `src/.vuepress/config.js` to `public`.
...
3. Deploy to surge, by typing `surge src/.vuepress/dist`.

You can also deploy to a [custom domain](http://surge.sh/help/adding-a-custom-domain) by adding `surge src/.vuepress/dist yourdomain.com`.

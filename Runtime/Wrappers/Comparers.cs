...
namespace Pixeye.Framework
{
	public static class Comparers
	{
		...
		sealed class EqualityBoundsInt : IEqualityComparer<BoundsInt>
		{
			...
			{
				return EqualityVector3Int.Default.Equals(self.position, vector.position) && EqualityVector3Int.Default.Equals(self.size, vector.size);
			}

			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public int GetHashCode(BoundsInt obj)
			{
				return EqualityVector3Int.Default.GetHashCode(obj.position) ^ EqualityVector3Int.Default.GetHashCode(obj.size) << 2;
			}
